    ...
    if (mobile.length > 0) {
        if (isMobilePhone(mobile)) {
            ...
            if (pwd.length < 1) $("#msg-password").addClass("show");
                ...
                $.ajax({
                    url: "http://sso.youyoucode.cn/api/users/sighup",
                    type: "post",
                    data: { username: mobile, email: "", password: pwd, mobilePhone: mobile, code: sms },
                    dataType: "json",
                    success: function(data) {
                        window.location.replace("http://www.youyoucode.cn/login/");
                    },
                    error: function(err) {
...
document.getElementById("js-sms-send-btn").onclick = function(e) {
    ...
    if (isMobilePhone(mobile)) {
        $.ajax({
            url: "http://sso.youyoucode.cn/api/VerifyCodes/create-for-sms",
            type: "post",
